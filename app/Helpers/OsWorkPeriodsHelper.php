...
class OsWorkPeriodsHelper
{
    ...
    public static function set_default_working_hours()
    {
        ...
        if (!self::$existing_work_periods) {
            ...
            if (self::$existing_work_periods) {
                self::$existing_work_periods = array_map(function ($work_period) {
                    return $work_period['week_day'];
                }, self::$existing_work_periods);
    ...
    public static function generate_days_with_custom_schedule($args = array())
    {
        ...
        if ($work_periods && isset($work_periods[0])) {
            ...
            if ($date->format('Y') != date('Y')) $html .= '<div class="os-form-sub-header sub-level"><h3>' . $date->format('Y') . '</h3></div>';
        }
        $chained_periods = [];
                ...
                if ($chained_periods) {
                    ...
                    foreach ($chained_periods as $chain_id => $chained_period) {
                        ...
                        if ($processing_year != $range_start_date->format('Y')) {
                            $html .= '</div><div class="os-form-sub-header sub-level"><h3>' . $range_start_date->format('Y') . '</h3></div><div class="custom-day-work-periods">';
                            $processing_year = $range_start_date->format('Y');
                        }
                        $html .= '<div class="custom-day-work-period is-range">';
                        $html .= '<a href="#" title="' . __('Edit Date Range Schedule', 'latepoint') . '" class="edit-custom-day" ' . self::generate_custom_day_period_action($range_start_date->format('Y-m-d'), false, array_merge($args, ['chain_id' => $chain_id])) . '><i class="latepoint-icon latepoint-icon-edit-3"></i></a>';
                        $html .= '<a href="#" data-os-pass-this="yes" data-os-after-call="latepoint_custom_day_removed" data-os-action="' . OsRouterHelper::build_route_name('settings', 'remove_chain_schedule') . '" data-os-params="' . OsUtilHelper::build_os_params(['chain_id' => $chain_id]) . '" data-os-prompt="' . __('Are you sure you want to remove custom schedule for this date range?', 'latepoint') . '" title="' . __('Remove Date Range Schedule', 'latepoint') . '" class="remove-custom-day"><i class="latepoint-icon latepoint-icon-trash-2"></i></a>';
                        $html .= '<div class="custom-day-work-period-i">';
                        $html .= '<div class="custom-day-number">' . $range_start_date->format('d') . ' - ' . $range_end_date->format('d') . '</div>';
                        if ($range_start_date->format('n') != $range_end_date->format('n')) {
                            $html .= '<div class="custom-day-month">' . OsUtilHelper::get_month_name_by_number($range_start_date->format('n')) . '-' . OsUtilHelper::get_month_name_by_number($range_end_date->format('n')) . '</div>';
                        } else {
                            $html .= '<div class="custom-day-month">' . OsUtilHelper::get_month_name_by_number($range_start_date->format('n')) . '</div>';
                        }
                        $html .= '</div>';
            ...
            foreach ($work_periods as $work_period) {
                ...
                if ($chained_periods) {
                    ...
                    foreach ($chained_periods as $chain_id => $chained_period) {
                        ...
                        $html .= '<a href="#" data-os-pass-this="yes" data-os-after-call="latepoint_custom_day_removed" data-os-action="' . OsRouterHelper::build_route_name('settings', 'remove_chain_schedule') . '" data-os-params="' . OsUtilHelper::build_os_params(['chain_id' => $chain_id]) . '" data-os-prompt="' . __('Are you sure you want to remove day off range?', 'latepoint') . '" title="' . __('Remove Day Off Range', 'latepoint') . '" class="remove-custom-day"><i class="latepoint-icon latepoint-icon-trash-2"></i></a>';
                        $html .= '<div class="custom-day-work-period-i">';
                        $html .= '<div class="custom-day-number">' . $range_start_date->format('d') . ' - ' . $range_end_date->format('d') . '</div>';
                        if ($range_start_date->format('n') != $range_end_date->format('n')) {
                            $html .= '<div class="custom-day-month">' . OsUtilHelper::get_month_name_by_number($range_start_date->format('n')) . '-' . OsUtilHelper::get_month_name_by_number($range_end_date->format('n')) . '</div>';
                ...
                if (empty($work_period->chain_id)) {
                    ...
                    $html .= '<a href="#" title="' . __('Edit Day Schedule', 'latepoint') . '" class="edit-custom-day" ' . self::generate_custom_day_period_action($work_period->custom_date, false, $args) . '><i class="latepoint-icon latepoint-icon-edit-3"></i></a>';
                    $html .= '<a href="#" data-os-pass-this="yes" data-os-after-call="latepoint_custom_day_removed" data-os-action="' . OsRouterHelper::build_route_name('settings', 'remove_custom_day_schedule') . '" data-os-params="' . OsUtilHelper::build_os_params(array_merge($args, ['date' => $work_period->custom_date])) . '" data-os-prompt="' . __('Are you sure you want to remove this day off?', 'latepoint') . '" title="' . __('Remove Day Off', 'latepoint') . '" class="remove-custom-day"><i class="latepoint-icon latepoint-icon-trash-2"></i></a>';
                    $html .= '<div class="custom-day-work-period-i">';
                    $html .= '<div class="custom-day-number">' . $date->format('d') . '</div>';
                    $html .= '<div class="custom-day-month">' . OsUtilHelper::get_month_name_by_number($date->format('n')) . '</div>';
                    $html .= '</div>';
        ...
        for ($i = 1; $i <= 7; $i++) {
            ...
            <div class="weekday-schedule-w <?php echo $is_day_off ? 'day-off' : ''; ?>">
                <div class="ws-head-w">
                    ...
                    <div class="ws-head">
                        <div
                            class="ws-day-name"><?php echo OsBookingHelper::get_weekday_name_by_number($i, true); ?></div>
                ...
                <div class="weekday-schedule-form">
                    ...
                    <div class="ws-period-add" data-
                         data-os-params="<?php echo OsUtilHelper::build_os_params($params); ?>"
                        ...
                        <div
                            class="add-period-label"><?php echo sprintf(__('Add another work period for %s', 'latepoint'), OsBookingHelper::get_weekday_name_by_number($i, true)); ?></div>
                    </div>
                </div>
            </div>
            <?php
