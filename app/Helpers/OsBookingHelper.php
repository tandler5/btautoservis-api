...
class OsBookingHelper
{
    ...
    public static function get_bookings_for_select($should_be_in_future = false)
    {
        ...
        if ($should_be_in_future) $bookings = $bookings->should_be_in_future();
        ...
        foreach ($bookings as $booking) {
            ...
            $bookings_options[] = ['value' => $booking->id, 'label' => $name];
        }
     ...
     * @return array[]
     */
    public static function generate_price_breakdown_rows(Booking $booking, bool $force_recalculate = false, array $rows_to_hide = []): array
    {
        $rows = [
            'before_subtotal' => [],
            'subtotal' => [],
    ...
    public static function count_pending_bookings($agent_id = false, $location_id = false)
    {
        ...
        return $bookings->where(['status IN' => OsBookingHelper::get_booking_statuses_for_pending_page()])->count();
    }
        ...
        if ($services && is_array($services) && !empty($services)) { ?>
            <div class="os-services os-animated-parent os-items os-as-rows os-selectable-items">
                <?php foreach ($services as $service) {
                    ...
                    ?>
                    <div
                        class="os-animated-child os-item os-selectable-item <?php echo ($preselected_service && $service->id == $preselected_service->id) ? 'selected is-preselected' : ''; ?> <?php echo ($is_priced) ? 'os-priced-item' : ''; ?> <?php if ($service->short_description) echo 'with-description'; ?> <?php if ($service->get_extra_durations()) echo 'has-multiple-durations has-child-items'; ?>"
                        ...
                        <div class="os-service-selector os-item-i os-animated-self"
                            ...
                            <?php if ($service->selection_image_id) { ?>
                                <span class="os-item-img-w"
                                      style="background-image: url(<?php echo $service->selection_image_url; ?>);"></span>
                            <?php } ?>
                ...
                <?php } ?>
                  ...
                  <?php } ?>
                        ...
                        <?php if (count($service_durations) > 1) { ?>
                            <div
                                class="os-service-durations os-animated-parent os-items os-as-grid os-three-columns os-selectable-items">
                                <?php
                                ...
                                foreach ($service_durations as $service_duration) {
                                    ...
                                    <div
                                        class="os-duration-item os-animated-child os-selectable-item os-item with-floating-price <?php echo ($is_priced) ? 'os-priced-item' : ''; ?>"
                                        ...
                                        <div class="os-animated-self os-item-i os-service-duration-selector">
                                            <?php if (($service_duration['duration'] >= 60) && !OsSettingsHelper::is_on('steps_show_duration_in_minutes')) { ?>
                                                ...
                                                <div
                                                    class="os-duration-label"><?php echo ($hours > 1) ? __('Hours', 'latepoint') : __('Hour', 'latepoint'); ?></div>
                                                <?php if ($minutes) echo '<div class="os-duration-sub-label"><span>' . $minutes . '</span> ' . __('Minutes', 'latepoint') . '</div>'; ?>
                                            <?php } else { ?>
                                                <div
                                                    class="os-duration-value"><?php echo $service_duration['duration']; ?></div>
                                                <div
                                                    class="os-duration-label"><?php _e('Minutes', 'latepoint'); ?></div>
                                            <?php } ?>
            ...
            foreach ($service_categories as $service_category) { ?>
                ...
                if ($count_child_categories || count($services)) {
                    ...
                    <div class="os-item-category-w os-items os-as-rows os-animated-child"
                         data-id="<?php echo $service_category->id; ?>">
                        <div class="os-item-category-info-w os-item os-animated-self with-plus">
                            <div class="os-item-category-info os-item-i">
                                <div class="os-item-img-w"
                                     style="background-image: url(<?php echo $service_category->selection_image_url; ?>);"></div>
                                <div class="os-item-name-w">
                                    <div class="os-item-name"><?php echo $service_category->name; ?></div>
                                </div>
                                <?php if (count($services)) { ?>
                                    <div class="os-item-child-count">
                                        <span><?php echo count($services); ?></span> <?php _e('Services', 'latepoint'); ?>
                                    </div>
                                <?php } ?>
                            </div>
                        </div>
