...
class ResourceHelper
{
     ...
     *
     */
    public static function get_resources_grouped_by_day(BookingRequest $booking_request, DateTime $date_from, DateTime $date_to = null, array $settings = []): array
    {
        $defaults = [
        ...
        foreach ($connections as $connection) {
            $resources[] = BookingResource::create_from_connection($connection);
        }
        if (empty($date_to)) $date_to = clone $date_from;
        ...
        $filter = new Filter(['connections' => $connections, 'date_from' => $date_from->format('Y-m-d'), 'date_to' => $date_to->format('Y-m-d')]);
        ...
        // Booked periods
        $booked_periods_filter = new Filter();
        ...
        if ($settings['exclude_booking_ids']) $booked_periods_filter->exclude_booking_ids = $settings['exclude_booking_ids'];
        ...
        for ($day_date = clone $date_from; $day_date->format('Y-m-d') <= $date_to->format('Y-m-d'); $day_date->modify('+1 day')) {
            ...
            foreach ($resources as $resource) {
                ...
                foreach ($weekday_periods[$day_date->format('N')] as $period) {
                    if ($period->custom_date && ($period->custom_date != $day_date->format('Y-m-d'))) continue; // if this period has a custom date set and if it doesn't match the one we search for - skip it
                    ...
                    if ((!$period->agent_id || $period->agent_id == $resource->agent_id) && (!$period->service_id || $period->service_id == $resource->service_id) && (!$period->location_id || $period->location_id == $resource->location_id)) {
                        if ($last_added_period) {
                            ...
                            if ($last_added_period->weight != $period->weight) break;
                            ...
                            $available_work_periods_groups[$group_index][] = TimePeriod::create_from_work_period($period);
                ...
                foreach ($booked_periods[$day_date->format('Y-m-d')] as $booked_period) {
                    if ($day_resource->agent_id == $booked_period->agent_id) {
                        if (OsSettingsHelper::is_on('one_location_at_time') || $day_resource->location_id == $booked_period->location_id) {
                            // block it if it's the same location or if agent can be present in one location only
                            $day_resource->add_booked_period($booked_period);
                        } elseif (!OsSettingsHelper::is_on('multiple_services_at_time') || $day_resource->service_id == $booked_period->service_id) {
                            // block it if it's the same service OR if agent is not allowed to perform a different service at the same time
                            $day_resource->add_booked_period($booked_period);
                        }
                    }
                }
    ...
    /**
     * @param BookingResource[] $resourcesLocation
     * @return array
     */
     ...
     * @return TimePeriod
     */
    public static function get_work_boundaries_for_groups_of_resources(array $groups_of_resources): TimePeriod
    {
        $times = [];
