...
use App\Helpers\OsSettingsHelper;
...
class BookingResource
{
    ...
    protected function retrieve_service_data()
    {
        ...
        if (!$service) {
            throw new \Exception("Služba s ID {$this->service_id} nebyla nalezena.");
        }
        $this->max_capacity = $service->capacity_max;
        $this->timeblock_interval = $service->timeblock_interval ? intval($service->timeblock_interval) : 60;
    ...
    public static function create_from_connection($connection): BookingResource
    {
